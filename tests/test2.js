...
for (i = 0; i < data.partitions.length; i++) {
    ...
    var pRect = createRectChild(tRect, partition.pos_x - data.offset_x, partition.pos_y - data.offset_y, partition.size_x, partition.size_y, 1);
    for (j = 0; j < partition.boxes.length; j++) {
        var box = partition.boxes[j];
        var bRect = createRectChild(pRect, box.pos_x - partition.offset_x, box.pos_y - partition.offset_y, box.size_x, box.size_y, 1);
        for (k = 0; k < box.modules.length; k++) {
            var mod = box.modules[k];
            var mRect = createRectChild(bRect, mod.pos_x - box.offset_x, mod.pos_y - box.offset_y, mod.size_x, mod.size_y, 1);
