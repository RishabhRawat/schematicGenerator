...
terminal::terminal(const terminal& baseTerminal, const int highIndex, const int lowIndex, const bool highToLow)
	: terminalIdentifier(
	  ...
	  type(baseTerminal.type),
	  terminalWidth(highIndex - lowIndex + 1),
	  parentModule(baseTerminal.parentModule),
	  systemTerminal(baseTerminal.systemTerminal),
	  highIndex(highIndex),
	  lowIndex(lowIndex),
	  highToLow(highToLow) {
	if (highIndex < lowIndex)
